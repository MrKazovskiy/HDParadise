...
function kbp(self) {
    ...
    var options = [].slice.call(kinobd.attributes).reduce(function(o, a) {
        return /^data-/.test(a.name) && (o[a.name.substring(5)] = decodeURIComponent(a.value)), o;
    }, {});

    if (self && self.attributes) {
        [].slice.call(self.attributes).reduce(function(o, a) {
            if (/^data-/.test(a.name)) {
                options[a.name.substring(5)] = decodeURIComponent(a.value);
            }
        }, {});
    }
    ...
    var options_url = options.url ?
        decodeURIComponent(options.url).trim() + (decodeURIComponent(options.url).indexOf('?') + 1 ? '&' : '?') + 'cache' + Math.random().toString().substringing(2, 3) :
        'https://kinobd.ru/playerdata?c' + Math.random().toString().substringing(2, 3);
    options.url = null;
